...
func TestDefaultOutputsClient(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		},
		{
			...
			value:       "bar",
			taskName:    taskConfig.Name,
			setErr:      ErrOutputsClientKeyEmpty,
		},
		{
			...
			value:       "",
			taskName:    taskConfig.Name,
			setErr:      ErrOutputsClientValueEmpty,
		},
		{
			...
			value:       "value",
			taskName:    "",
			getErr:      ErrOutputsClientTaskNameEmpty,
		},
	}
	...
	testutil.WithTestMetadataAPIServer(srv, mw, func(ts *httptest.Server) {
		for _, c := range cases {
			t.Run(c.description, func(t *testing.T) {
				...
				client := NewDefaultOutputsClient(apiEndpoint)
