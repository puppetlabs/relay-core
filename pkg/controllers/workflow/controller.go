...
import (
	...
	teklisters "github.com/tektoncd/pipeline/pkg/client/listers/pipeline/v1beta1"
...
func (c *Controller) processPipelineRun(ctx context.Context, key string) error {
	...
	plr, err := c.tekclient.TektonV1beta1().PipelineRuns(namespace).Get(name, metav1.GetOptions{})
...
func (c *Controller) uploadLogs(ctx context.Context, plr *tekv1beta1.PipelineRun) (map[string]string, error) {
	logAnnotations := make(map[string]string, 0)
...
func (c *Controller) handleWorkflowRun(ctx context.Context, wr *nebulav1.WorkflowRun) error {
	...
	if wr.ObjectMeta.DeletionTimestamp.IsZero() {
		...
		if annotation, ok := wr.GetAnnotations()[pipelineRunAnnotation]; !ok && !cancelled {
			...
			plr, err := c.tekclient.TektonV1beta1().PipelineRuns(wr.GetNamespace()).Get(annotation, metav1.GetOptions{})
			if k8serrors.IsNotFound(err) {
				return nil
			}
			if err != nil {
			...
			_, err = c.tekclient.TektonV1beta1().PipelineRuns(wr.GetNamespace()).Update(plr)
			if err != nil {
				return err
			}
		}
...
func (c *Controller) initializeWorkflowRun(ctx context.Context, wr *nebulav1.WorkflowRun) error {
	for name, value := range wr.State.Steps {
		...
		err := c.writeWorkflowState(wr, taskHash, value)
		if err != nil {
			return err
		}
	}
...
func (c *Controller) createPipelineRun(wr *nebulav1.WorkflowRun) (*tekv1beta1.PipelineRun, error) {
	...
	plr, err := c.tekclient.TektonV1beta1().PipelineRuns(namespace).Get(wr.GetName(), metav1.GetOptions{})
	if plr != nil && plr != (&tekv1beta1.PipelineRun{}) && plr.Name != "" {
		return plr, nil
	}
	...
	createdPipelineRun, err := c.tekclient.TektonV1beta1().PipelineRuns(namespace).Create(pipelineRun)
	if err != nil {
		return nil, err
	}
...
func (c *Controller) updateWorkflowRunStatus(plr *tekv1beta1.PipelineRun, wr *nebulav1.WorkflowRun) (*nebulav1.WorkflowRunStatus, error) {
...
func (c *Controller) initializePipeline(wr *nebulav1.WorkflowRun, service *corev1.Service) errors.Error {
	...
	pipeline, err := c.tekclient.TektonV1beta1().Pipelines(wr.GetNamespace()).Get(wr.GetName(), metav1.GetOptions{})
	...
	pipeline, err = c.createPipeline(wr, pipelineTasks)
	if err != nil {
		return errors.NewWorkflowExecutionError().WithCause(err)
	}
...
func (c *Controller) createTasks(wr *nebulav1.WorkflowRun, service *corev1.Service) (StepTasks, errors.Error) {
	...
	for _, step := range wr.Spec.Workflow.Steps {
		...
		err := c.createTaskConfigMap(taskHash, wr, step)
		if err != nil {
			return nil, errors.NewWorkflowExecutionError().WithCause(err)
		}

		_, err = c.createTaskFromStep(wr, taskHash, metadataAPIURL, step)
		if err != nil {
			return nil, errors.NewWorkflowExecutionError().WithCause(err)
		}
		...
		if step.When.Value() != nil {
			err := c.createCondition(wr, taskHash, metadataAPIURL)
			if err != nil {
				return nil, errors.NewWorkflowExecutionError().WithCause(err)
			}
...
func (c *Controller) createPipelineTasks(stepTasks StepTasks) ([]tekv1beta1.PipelineTask, errors.Error) {
...
func (c *Controller) getTaskDependencies(stepTask StepTask) ([]string, []tekv1beta1.PipelineTaskCondition, errors.Error) {
...
func (c *Controller) createCondition(wr *nebulav1.WorkflowRun, taskHash task.Hash, metadataAPIURL string) errors.Error {
	...
	_, err := c.tekclient.TektonV1alpha1().Conditions(wr.GetNamespace()).Create(&condition)
	if err != nil && !k8serrors.IsAlreadyExists(err) {
		return errors.NewWorkflowExecutionError().WithCause(err)
	}
...
func (c *Controller) createTaskConfigMap(taskHash task.Hash, wr *nebulav1.WorkflowRun, step *nebulav1.WorkflowStep) errors.Error {
	configMapData, _ := getConfigMapData(wr.Spec.Workflow.Parameters, wr.Spec.Parameters, step)
	_, err := c.createConfigMap(wr, taskHash, configMapData)
	if err != nil {
		return errors.NewWorkflowExecutionError().WithCause(err)
	}
...
func (c *Controller) createTaskFromStep(wr *nebulav1.WorkflowRun, taskHash task.Hash, metadataAPIURL string, step *nebulav1.WorkflowStep) (*tekv1beta1.Task, errors.Error) {
	variantStep := step
	container, volumes := getTaskContainer(metadataAPIURL, taskHash, variantStep)
	return c.createTask(wr, taskHash, container, volumes)
}

func (c *Controller) createTask(wr *nebulav1.WorkflowRun, taskHash task.Hash, container *corev1.Container, volumes []corev1.Volume) (*tekv1beta1.Task, errors.Error) {
	taskHashKey := taskHash.HexEncoding()
	...
	task, err := c.tekclient.TektonV1beta1().Tasks(wr.GetNamespace()).Create(task)
	if k8serrors.IsAlreadyExists(err) {
		task, err = c.tekclient.TektonV1beta1().Tasks(wr.GetNamespace()).Get(taskHashKey, metav1.GetOptions{})
	}
	if err != nil {
		return nil, errors.NewWorkflowExecutionError().WithCause(err)
	}
...
func (c *Controller) createPipeline(wr *nebulav1.WorkflowRun, pipelineTasks []tekv1beta1.PipelineTask) (*tekv1beta1.Pipeline, errors.Error) {
	...
	pipeline, err := c.tekclient.TektonV1beta1().Pipelines(wr.GetNamespace()).Create(pipeline)
	if k8serrors.IsAlreadyExists(err) {
		pipeline, err = c.tekclient.TektonV1beta1().Pipelines(wr.GetNamespace()).Get(wr.GetName(), metav1.GetOptions{})
	}
	if err != nil {
...
func (c *Controller) createConfigMap(wr *nebulav1.WorkflowRun, taskHash task.Hash, data map[string]string) (*corev1.ConfigMap, error) {
	taskHashKey := taskHash.HexEncoding()
	...
	configMap, err := c.kubeclient.CoreV1().ConfigMaps(wr.GetNamespace()).Create(configMap)
	if k8serrors.IsAlreadyExists(err) {
		configMap, err = c.kubeclient.CoreV1().ConfigMaps(wr.GetNamespace()).Get(taskHashKey, metav1.GetOptions{})
	}
	if err != nil {
		return nil, errors.NewWorkflowExecutionError().WithCause(err)
	}
...
func createMetadataAPIPod(kc kubernetes.Interface, image string, saccount *corev1.ServiceAccount,
	...
	pod, err := kc.CoreV1().Pods(wr.GetNamespace()).Create(pod)
	if k8serrors.IsAlreadyExists(err) {
		pod, err = kc.CoreV1().Pods(wr.GetNamespace()).Get(name, metav1.GetOptions{})
	}
	if err != nil {
		return nil, err
	}
...
func createMetadataAPIService(kc kubernetes.Interface, wr *nebulav1.WorkflowRun) (*corev1.Service, error) {
	...
	service, err := kc.CoreV1().Services(wr.GetNamespace()).Create(service)
	if k8serrors.IsAlreadyExists(err) {
		service, err = kc.CoreV1().Services(wr.GetNamespace()).Get(name, metav1.GetOptions{})
	}
	if err != nil {
		return nil, err
	}
